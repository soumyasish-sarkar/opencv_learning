...
def draw(event,x,y,flags,param):
    ...
    if(event == 1):
        ...
        pt1y=y;
         ...
         if(flag==1):
            img_copy = img.copy()
            cv2.rectangle(img_copy, pt1=(pt1x,pt1y), pt2=(x, y), color=(0, 255, 255),thickness=2)  # thickness=-1 for fill the image
        ...
        flag = 0
        cropped = img[min(pt1y, y):max(pt1y, y), min(pt1x, x):max(pt1x, x)]

        #show cropped image if cropped size is not empty
        if(cropped.size>0):
...
img_ip = cv2.imread(r"C:\Users\soumyasish-sarkar\PycharmProjects\opencv_learning\image\tinkesh.jpg")
